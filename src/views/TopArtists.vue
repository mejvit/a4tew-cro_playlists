...
<div>
    <div class="container">
        ...
        <div class="my-3 alert alert-success">Nejhranější interpreti od {{ myDate.start | date_cz_format}} do {{ myDate.end | date_cz_format}}</div>
    </div>
    <div class="container">
    <div class="row">
        <div class="col-sm-6">
            <h2>Celostátní stanice</h2>
            <top-artists-station v-for="station in nationwideStations" :dateRange="dateRange" v-bind:key="station.id" v-bind:name="station.name" v-bind:slug="station.slug" :dateFrom="myDate.start" :dateTo="myDate.end"  /> 
        </div>
        <div class="col-sm-6">
            <h2>Regionální stanice</h2>
            <top-artists-station v-for="station in regionalStations" :dateRange="dateRange" v-bind:key="station.id" v-bind:name="station.name" v-bind:slug="station.slug" :dateFrom="myDate.start" :dateTo="myDate.end"/>  
        </div> 
...
<script>
import TopArtistsStation from '@/components/TopArtistsStation.vue'

export default {
    name: 'TopArtists',
    components: {
        'top-artists-station': TopArtistsStation
    },
  data () {
    return {
      date: null,
